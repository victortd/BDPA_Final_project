   ...
   public static class docsperwordReducer extends Reducer<Text, Text, Text, DoubleWritable> {
      ...
      public void reduce(Text key, Iterable<Text> values, Context context)
  		...
  		for (int i = 0; i < contents.size(); i++) {
  			...
  			double thiswordcount = Integer.parseInt(contentlist[1]);
  			double wordcountperdoc = Integer.parseInt(contentlist[2]);
  			double tfidf = (thiswordcount/wordcountperdoc)*Math.log(2 / docsperword);
  			
  			context.write(new Text(word+";"+contentlist[0]), new DoubleWritable(tfidf));    
           }
      }
